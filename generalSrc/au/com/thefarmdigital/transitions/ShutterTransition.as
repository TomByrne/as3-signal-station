package au.com.thefarmdigital.transitions
{
	...
	public class ShutterTransition extends Transition
	{
		...
		public var shutterDirection:String;
		...
		override public function doTransition(start:DisplayObject, finish:DisplayObject, bitmap:Bitmap, duration:Number, currentTime:Number):void{
			...
			for(var i:int=0; i<length; ++i){
				...
				if(ver){
					...
					rect.width *= 1-pos;
					matrix.a *= 1-pos;
					matrix.tx = bounds.x+shutter.rect.x*pos;
					matrix.ty = bounds.y-(bitmap.width*(rect.x/bitmap.width)*angle);
				}else{
					matrix.c = angle*matrix.a;
					rect.height *= 1-pos;
					matrix.d *= 1-pos;
					matrix.ty = bounds.y+shutter.rect.y*pos;
					matrix.tx = bounds.x-(bitmap.height*(rect.y/bitmap.height)*angle);
				...
				matrix = subject.transform.matrix;
				bounds = subject.getBounds(bitmap);
				angle = Math.tan(-Math.PI/4*tilt*(1-pos));
				if(ver){
					rect.width *= pos;
					rect.x += shutter.rect.width*(1-pos);
					matrix.a *= pos;
					matrix.tx = bounds.y+shutter.rect.x*(1-pos)+(shutter.rect.width*(1-pos));
					...
					matrix.d *= pos;
					matrix.ty = bounds.y+shutter.rect.y*(1-pos)+(shutter.rect.height*(1-pos));
